...
import atomai
from atomai.nets import GPRegressionModel, fcFeatureExtractor, CustomGPModel
...
class dklGPTrainer(GPTrainer):
    ...
    def compile_multi_model_trainer(self,
        ...
        self.gp_model.to(self.device).to(self.device)
        self.likelihood.to(self.device).to(self.device)
